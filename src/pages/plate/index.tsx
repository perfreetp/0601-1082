...
const PlatePage: React.FC = () => {
  ...
  const [mealFoods, setMealFoods] = useState<Record<string, FoodItem[]>>({
    breakfast: [
  ...
  const handleAddFood = () => {
    const newFood = { ...mockFoodDatabase[Math.floor(Math.random() * 5) + 5] };
    setMealFoods((prev) => {
      const foods = [...prev[activeMeal], newFood];
      return { ...prev, [activeMeal]: foods };
    });
